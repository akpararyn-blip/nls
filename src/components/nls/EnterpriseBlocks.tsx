...
export function PowerSection() {
  ...
  const items: CardItem[] = [
    { Icon: BatteryCharging, h: t("Автономный дизель-генератор", "Автономды дизельді генератор"), p: t("Мощность 380 кВт со встроенным баком — до 8 часов бесперебойной работы под нагрузкой.", "Кірістірілген багы бар 380 кВт қуат — жүктеме кезінде 8 сағатқа дейін үздіксіз жұмыс.") },
  ...
  return (
    <EnterpriseSection
      ...
      subtitle={t("Многоуровневая архитектура питания N+1: два независимых ввода, ИБП и резервная генерация.", "Көп деңгейлі N+1 қуат сәулеті: екі тәуелсіз кіріс, ҮҚК және резервтік генерация.")}
      items={items}
    />
  );
}
...
export function ClimateSection() {
  const t = useT();
  const items: CardItem[] = [
    { Icon: Snowflake, h: t("Изолированный холодный коридор", "Оқшауланған суық дәліз"), p: t("Cold Aisle Containment со слайд-дверьми и автоматическим открытием потолка при инцидентах.", "Слайд-есіктері бар Cold Aisle Containment және оқиға кезінде төбенің автоматты ашылуы.") },
...
export function SlaSection() {
  ...
  const stats = [
    ...
    { value: "N+1", label: t("Отказоустойчивость Tier III", "Tier III істен шықпауы"), text: t("Параллельные пути электропитания и охлаждения, плановые работы без остановки сервисов.", "Электр қуаты мен салқындатудың параллель жолдары, сервистерді тоқтатпай жоспарлы жұмыстар.") },
  ];
  return (
